 ...
 */
?>
<?php do_action('stronghold_blog_layout_class_wrapper_before'); ?>
	...
	if( $featured_image ) : ?>
		<div class="post-thumb blog-thumb">
			...
			if( function_exists( 'stronghold_featured_image' ) ) :
				stronghold_featured_image();
			endif;
			?>
	    </div>
	<?php endif; ?> 

	<?php do_action('stronghold_before_entry_header'); ?>
			...
			<?php the_title( sprintf( '<h1 class="entry-title"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h1>' ); ?>
				...
				<?php if ( get_theme_mod('enable_single_post_top_meta', true ) ): ?>
					<footer class="entry-meta">
						<?php if(function_exists('stronghold_entry_top_meta') ) {
						    stronghold_entry_top_meta(); 
						} ?> 
					</footer><!-- .entry-footer -->
				<?php endif;?>  
		</header><!-- .entry-header -->
...
<?php do_action('stronghold_blog_layout_class_wrapper_after'); ?>